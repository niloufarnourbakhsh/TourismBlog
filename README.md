# Tourism Blog (Laravel)

A tourism blog platform built with Laravel where users can read travel posts
and admins can manage content.
## 🔹 About the Project
This project is a sample portfolio project developed to demonstrate my skills
as a Laravel developer, including CRUD operations, authentication, and clean architecture.
## 🔹 Features
- User authentication (Login / Register)
- Admin panel for managing blog posts
- Create, edit, delete tourism posts
- Image upload for posts
- Responsive UI
- ## 🔹 Technologies Used
- PHP 8+
- Laravel
- MySQL
- Blade
- Bootstrap / Tailwind
 ## 🔹 Installation
```bash
git clone https://github.com/your-username/TourismBlog.git
cd TourismBlog
composer install
cp .env.example .env
php artisan key:generate
php artisan migrate
php artisan serve
- ## 🔹 Aboute Me
Hi, I’m Niloufar.  
This is my Laravel project called Tourism Blog.  
 I built this project to demonstrate my backend skills including CRUD operations,
authentication, and clean project structure.
 I’m currently looking for a remote junior Laravel developer position.

